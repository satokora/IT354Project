...
function textscrape(){
    if (isset($_POST['sourceUrl'])){
      ...
      //$instructions = addcslashes($instructions,"\\\'\"\n\r");
      return $instructions;
...
function setIngreAndDirec()
{
  ?>
    ...
    $("#data-from-scratch").submit(function(event) {
      ...
      $("#photo_path").val($(".card img").attr("src"));
      });
    });
    </script>
    <?php
  ...
  if(isset($_POST['save'])){
    ...
    $imgdat=null;
      ...
      if (isset($_FILES["upload"]["tmp_name"]))
      {
        ...
        $imgdat = file_get_contents($_FILES["upload"]["tmp_name"]);
      }
    ...
    if($imgdat!=null){
        $result = $wpdb->update($wpdb->recipes, array('photo'=>$imgdat), array('id'=>$recipeId));
      }
...
function setRecipe()
{
  ...
  if(isset($_GET['id'])){
    ...
    if ($photoPath=="BLOB"){
          $photoPath = get_template_directory_uri()."/create-image.php?id=".$rows[0]->id;
    }
    ?>
    <script type="text/javascript">
    jQuery(document).ready(function ($) {
